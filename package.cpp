...
void startSnake(int rightState, int leftState, int downState, int upState);
...
void loop() {
  ...
  // 객체화)
    ...
    // 점프맨
    if (nowState == 1) {
      startSnake(rightState, leftState, downState, upState);
    }
    // 테트리스
    else if (nowState == 2) {
...
void memorizegame() {
  ...
  // countA for문
  countA++;
  // index // randomArray 배열에 어디와 비교할지 
  // 입력받는 상태
  int countB = 0;
  for (;;) {
    int upState = digitalRead(upPin);
...
void startSnake(int rightState, int leftState, int downState, int upState){
  ...
  //Check For Inputs
  if(rightState && snake.dir[1]==0){
    snake.dir[0] = 0;
    snake.dir[1] = -1;
  }else if(leftState && snake.dir[1]==0){
    snake.dir[0] = 0;
    snake.dir[1] = 1;
  }else if(downState && snake.dir[0]==0){
    snake.dir[0] = -1;
    snake.dir[1] = 0;
  }else if(upState && snake.dir[0]==0){
    snake.dir[0] = 1;
    snake.dir[1] = 0;
  }
